...
namespace usb_python2
{
	...
	int toysmarch_drumpad_device::read(std::vector<uint8_t>& buf, const size_t requestedLen)
	{
		...
		uint8_t state[9] = {
			stateIdx,
			p2dev->GetKeyState(L"ToysMarchP1Cymbal") * 128,
			p2dev->GetKeyState(L"ToysMarchP1DrumL") * 128,
			p2dev->GetKeyState(L"ToysMarchP1DrumR") * 128,
			0,
			p2dev->GetKeyState(L"ToysMarchP2Cymbal") * 128,
			p2dev->GetKeyState(L"ToysMarchP2DrumL") * 128,
			p2dev->GetKeyState(L"ToysMarchP2DrumR") * 128,
			0
		};
